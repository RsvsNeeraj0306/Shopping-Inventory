...
const NewInventoryItem = ({ sidebarOpen }) => {
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, height: '100%', width: sidebarOpen ? 'calc(100% - 240px)' : '100%' }}>

            <Box display={"flex"} justifyContent={"space-between"} alignContent={"center"} sx={{ backgroundColor: '#f5f5f5', marginTop: '10px', width: '100%', height: '36px' }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2}}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, height: '813px', borderRadius: '12px', width: sidebarOpen ? 'calc(100% - 240px)' : '100%' }}>
                    <FormSection />
                </Box>
